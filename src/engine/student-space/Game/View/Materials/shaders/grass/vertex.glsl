...
// student-space-v1: camera-distance fade. Blades collapse to base height
// as the camera pulls away, so a zoomed-out view shows a clean island
// instead of a fur-mat. Configured from Grass.js.
uniform float uCameraFadeNear;
uniform float uCameraFadeFar;
// Shared wind gust 0..1 from state.wind. Multiplied into the per-blade
// displacement so everything on the island swells / settles together.
...
void main()
{
    ...
    // Match the island mesh's miniature-planet bend so blade roots stay
    // planted on the rendered terrain instead of hovering above it.
    ...
    // student-space-v1: extra collapse when the orbit camera pulls back,
    // so a zoomed-out island doesn't read as a hairy fur-mat.
    ...
    // Wind — student-space-v1: slow scroll, dampened displacement, AND
    // modulated by the shared `uWindGust` envelope so the grass swells in
    // and out in sync with flowers / trees / particles. DESIGN.md
    // motion-rules envelope.
    vec2 noiseUv = modelPosition.xz * 0.02 + uTime * 0.024;
    vec4 noiseColor = texture2D(uNoiseTexture, noiseUv);
    float windAmp = 0.45 * uWindGust;
    modelPosition.x += (noiseColor.x - 0.5) * windAmp * tipness * scale;
    modelPosition.z += (noiseColor.y - 0.5) * windAmp * tipness * scale;
    ...
    // Grass color
    vec3 uGrassDefaultColor = vec3(0.29, 0.56, 0.25);
