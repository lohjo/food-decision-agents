...
import { getGrowthSummaryHandler } from '~/server/growth-summary.handler.server'

export const Route = createFileRoute('/api/growth/summary')({
  server: {
    handlers: {
...
async function handle(request: Request): Promise<Response> {
  ...
  try {
    const result = await getGrowthSummaryHandler({ year })
    ...
    console.error('[api/growth/summary] failed', err)
    return jsonError(500, 'internal_error', 'Failed to load growth summary.')
  }
}
