...
export const Route = createFileRoute('/api/share/revoke')({
  server: {
    handlers: {
...
async function handle(request: Request): Promise<Response> {
  if (!isSameOriginRequest(request)) {
    return jsonError(403, 'cross_origin', 'Share-revoke must originate from this site.')
  }
  let body: unknown = {}
  try {
  ...
  try {
    await revokeShareTokenHandler(body as never)
    ...
    if (isZodValidationError(err)) {
      return jsonError(400, 'invalid_input', 'Invalid revoke payload.')
    }
    console.error('[api/share/revoke] failed', err)
    return jsonError(500, 'internal_error', 'Failed to revoke share token.')
  }
}
